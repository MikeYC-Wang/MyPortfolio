...
- **CI/CD via GitHub Actions:**
  - Pushes to `main` that touch `frontend/**` auto-deploy via `.github/workflows/deploy.yml` (runs `type-check` + `build:ssg`, then publishes `frontend/dist` to `gh-pages` with `peaceiris/actions-gh-pages@v4`, `force_orphan: true`). Can also be triggered manually from the Actions tab (`workflow_dispatch`).
  - `frontend/deploy.sh` is still available as a manual fallback / escape hatch but is no longer the primary deploy path.
  - Pull requests targeting `main` run `.github/workflows/ci.yml` (lint + type-check jobs). Unit tests (`npm run test:unit`) are NOT wired in yet — the only existing spec is the default scaffolded `App.spec.ts` which would fail.
  - Backend on Render still auto-deploys from its own Render → Git integration, independent of GitHub Actions.
